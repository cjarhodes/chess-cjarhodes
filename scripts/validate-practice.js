...
requirePattern(app, /function placeDailySprintCard[\s\S]{0,700}coach-hero/, 'Daily Sprint must appear immediately after the Coach hero at every breakpoint');
...
requirePattern(app, /function renderCoachDailyPlan[\s\S]{0,2200}Today’s training complete[\s\S]{0,5000}Continue rehearsing[\s\S]{0,3600}Train .*then transfer[\s\S]{0,2800}Start your first coached session/, 'daily plan must cover completion, restored rehearsal, focused transfer, and first-time onboarding journeys');
...
requirePattern(smoke, /legacy move sprint was not preserved[\s\S]+legacy drill sprint was not preserved[\s\S]+one-click session[\s\S]+reload skipped an unfinished adaptive drill phase[\s\S]+restored adaptive drill session did not restart[\s\S]+focused drills did not advance into transfer play[\s\S]+adaptive takeaway did not assess live transfer[\s\S]+adaptive session completion did not survive reload[\s\S]+daily sprint progress leaked between browser accounts[\s\S]+revealed answer was not completed[\s\S]+stale engine result leaked[\s\S]+clean first visit did not offer a one-click training start[\s\S]+mobile first visit did not show the Daily Sprint before optional account setup[\s\S]+active baseline did not explain its in-progress state[\s\S]+review did not surface an actionable Coach cue[\s\S]+review did not offer one-click practice for the mistake[\s\S]+reviewed move did not advance the Daily Sprint[\s\S]+ten reviewed moves did not complete the Daily Sprint[\s\S]+move sprint completion did not persist across reload[\s\S]+review practice action did not open the drill/, 'browser smoke must cover upgrades, mid-phase reload, first-session onboarding, adaptive transfer journeys, persistence, isolation, mobile priority, races, and completion');
requirePattern(smoke, /setViewportSize\(\{ width: 390, height: 844 \}\)/, 'browser smoke must cover the narrow-viewport desktop notice');
