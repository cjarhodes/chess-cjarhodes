...
requirePattern(app, /function renderCoachDailyPlan[\s\S]{0,1800}Today’s training complete[\s\S]{0,3200}Train .*then transfer[\s\S]{0,2400}Build your 10-move baseline/, 'daily plan must cover completion, focused transfer, and first-time baseline journeys');
...
requirePattern(smoke, /legacy move sprint was not preserved[\s\S]+legacy drill sprint was not preserved[\s\S]+one-click session[\s\S]+focused drills did not advance into transfer play[\s\S]+adaptive takeaway did not assess live transfer[\s\S]+adaptive session completion did not survive reload[\s\S]+daily sprint progress leaked between browser accounts[\s\S]+revealed answer was not completed[\s\S]+stale engine result leaked[\s\S]+clean first visit did not offer a one-click baseline[\s\S]+mobile first visit did not show the Daily Sprint before optional account setup[\s\S]+reviewed move did not advance the Daily Sprint[\s\S]+ten reviewed moves did not complete the Daily Sprint[\s\S]+move sprint completion did not persist across reload/, 'browser smoke must cover upgrades, baseline and adaptive transfer journeys, persistence, isolation, mobile priority, races, and completion');
