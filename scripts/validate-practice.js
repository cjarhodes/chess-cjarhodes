...
requirePattern(smoke, /legacy move sprint was not preserved[\s\S]+legacy drill sprint was not preserved[\s\S]+one-click session[\s\S]+reload skipped an unfinished adaptive drill phase[\s\S]+restored adaptive drill session did not restart[\s\S]+focused drills did not advance into transfer play[\s\S]+adaptive takeaway did not assess live transfer[\s\S]+adaptive session completion did not survive reload[\s\S]+daily sprint progress leaked between browser accounts[\s\S]+revealed answer was not completed[\s\S]+stale engine result leaked[\s\S]+clean first visit did not offer a one-click training start[\s\S]+mobile first visit did not show the Daily Sprint before optional account setup[\s\S]+active baseline did not explain its in-progress state[\s\S]+review did not surface an actionable Coach cue[\s\S]+reviewed move did not advance the Daily Sprint[\s\S]+ten reviewed moves did not complete the Daily Sprint[\s\S]+move sprint completion did not persist across reload/, 'browser smoke must cover upgrades, mid-phase reload, first-session onboarding, adaptive transfer journeys, persistence, isolation, mobile priority, races, and completion');
