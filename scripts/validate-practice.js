...
requirePattern(app, /const DAILY_SPRINT_KEY = 'coach:daily-sprint:v1'[\s\S]{0,180}DAILY_MOVE_TARGET = 10[\s\S]{0,120}DAILY_DRILL_TARGET = 3/, 'Daily Sprint must define bounded move and drill targets');
...
requirePattern(app, /function recordDailySprintMove[\s\S]{0,1200}completedAt[\s\S]{0,1800}function recordDailySprintDrill[\s\S]{0,1400}takeaway/, 'Daily Sprint must complete from reviewed moves or drills with a takeaway');
requirePattern(app, /function orderedPracticeRunItems[\s\S]{0,500}slice\(0, DAILY_DRILL_TARGET\)/, 'Daily Sprint drill sessions must stay bounded');
...
requirePattern(html, /Daily Sprint · 5–10 min[\s\S]{0,700}id="daily-sprint-progress-label"[\s\S]{0,700}id="daily-sprint-takeaway"/, 'Daily Sprint must show its timebox, progress, and completion takeaway');
requirePattern(app, /function renderCoachDailyPlan[\s\S]{0,1800}Today’s training complete[\s\S]{0,2600}drill Daily Sprint[\s\S]{0,1800}10-move Daily Sprint/, 'daily plan must prioritize completion, active work, due drills, and a bounded coached sprint');
...
requirePattern(smoke, /one-click sprint[\s\S]+daily sprint did not produce a clear completion state[\s\S]+daily sprint completion did not survive reload[\s\S]+daily sprint progress leaked between browser accounts[\s\S]+revealed answer was not completed[\s\S]+stale engine result leaked[\s\S]+clean first visit did not offer a one-click sprint[\s\S]+mobile first visit did not show the Daily Sprint before optional account setup[\s\S]+reviewed move did not advance the Daily Sprint[\s\S]+ten reviewed moves did not complete the Daily Sprint[\s\S]+move sprint completion did not persist across reload/, 'browser smoke must cover both Daily Sprint journeys, persistence, account isolation, mobile priority, engine races, and completion');
