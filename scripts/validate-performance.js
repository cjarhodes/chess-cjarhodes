...
const syncMoveBlock = html.match(/async function syncRemoteCoachMove\(review\) \{[\s\S]*?\n\}/);
const deleteMoveBlock = html.match(/async function deleteRemoteCoachMove\(review\) \{[\s\S]*?\n\}/);
const threatBlock = html.match(/async function showThreats\(\) \{[\s\S]*?\n\}/);
...
requirePattern(html, /REMOTE_GAME_SYNC_DEBOUNCE_MS/, 'remote game updates must be debounced');
requirePattern(html, /function queueRemoteCoachGameUpdate/, 'remote game updates must use a queue helper');
requirePattern(html, /function upsertRemoteInsightEntry/, 'remote insights must be updated incrementally after move sync');
requirePattern(html, /function removeRemoteInsightEntry/, 'remote insights must be removable incrementally after takeback');
if (syncMoveBlock) {
...
requirePattern(html, /if\s*\(!artifacts\.drills\.length\s*&&\s*!artifacts\.cards\.length\)\s*return/, 'learning artifact sync must skip empty delete/upsert work');
...
requirePattern(html, /let threatRequestId\s*=/, 'threat scans must have a stale-position request guard');
requirePattern(html, /function renderThreatItems/, 'threat panel must render static threats independently');
if (threatBlock) {
